...
const { recordGenerationStat, loadAdminStats } = require("./adminStats");
const { canonicalHostRedirect } = require("./canonicalHost");
const { crawlerLogger } = require("./crawlerLog");
const { injectReportBadge, takenDownPage, registerReportRoutes } = require("./reports");
const {
  STARTER_MODEL,
  STARTER_MODEL_LABEL,
  PAID_MODEL_LABEL,
...
const { classifyActivation, shouldSendPaymentFailedEmail } = require("./emailDecisions");
...
const { registerSignupRoute } = require("./signup");
const {
...
// Shared with the browser: one definition of what a spec is and how it reaches
// the prompt. See public/spec.js.
const appSpec = require("../public/spec.js");
...
const app = express();
// Don't advertise the server framework (flagged by VibeSafe's live scan).
...
app.use((req, res, next) => {
  ...
  // API responses can carry per-user data (plan status, the admin user list), and
  // without this Vercel marks them "public", which lets shared caches keep them.
  // Static marketing pages are identical for everyone, so they keep the default.
  // Routes that need something else (the SSE stream) set their own value later.
  if (req.path.startsWith("/api/")) res.setHeader("Cache-Control", "no-store");
  next();
});

// Duplicate copies of the site on apps.<domain> and the .vercel.app alias now
// redirect to the real domain. Registered before express.static so static pages
// are covered too. See lib/canonicalHost.js.
app.use(
  canonicalHostRedirect({
    canonicalHost: new URL(SITE_URL).host,
    appsHost: process.env.ISOLATED_APPS_HOST,
    isProduction: process.env.VERCEL_ENV === "production",
  })
);

// One log line per search-engine or AI crawler visit, so "when were we last
// crawled?" is answerable from the runtime logs instead of waiting days for
// Search Console. Registered before express.static so crawls of the .html
// pages are counted, not just the routes Express handles itself.
// Records bot name, path, status and time only — see lib/crawlerLog.js.
...
app.post("/api/stripe-webhook", express.raw({ type: "application/json" }), async (req, res) => {
  ...
  try {
    if (event.type === "checkout.session.completed") {
      ...
      // .select() on the update returns the row as it now stands, but what we need
      // to decide whether to email is whether this transitioned FROM active — so
      ...
      // Only the definitive "it's over" event, and only on a real active->ended
      // transition — not customer.subscription.updated wobbling between trialing/
      // active/etc, and not a duplicate delivery of a cancellation already handled.
      if (event.type === "customer.subscription.deleted" && before?.status === "active" && status === "canceled" && before.user_id) {
        const { data: userData, error: userErr } = await supabaseAdmin.auth.admin.getUserById(before.user_id);
        if (userErr) console.error("[stripe-webhook] could not look up user for cancellation email", before.user_id, userErr);
        else if (userData?.user?.email) {
          const name = userData.user.user_metadata?.full_name || userData.user.user_metadata?.name || null;
          const result = await sendEmail({
            to: userData.user.email,
            subject: "Your Managed plan has ended — VibeSafe Builder",
            html: subscriptionCanceledEmailHtml({ name }),
          });
          if (!result.ok) console.error("Could not send cancellation email to", userData.user.email, result.error);
        }
      }
    } else if (event.type === "invoice.payment_failed") {
      const invoice = event.data.object;
      if (shouldSendPaymentFailedEmail(invoice)) {
        const email = invoice.customer_email
          || (await stripe.customers.retrieve(invoice.customer).catch(() => null))?.email;
        if (email) {
          const result = await sendEmail({
            to: email,
            subject: "We couldn't process your payment — VibeSafe Builder",
            html: paymentFailedEmailHtml({ name: null }),
          });
          if (!result.ok) console.error("Could not send payment-failed email to", email, result.error);
        }
      }
...
async function activateManagedAccess(email, stripeCustomerId) {
  ...
  // Read the PRIOR status before overwriting it — this is what classifyActivation
  // needs to tell "genuinely new", "reactivating after a lapse", and "already
  // active" apart. checkout.session.completed, customer.subscription.created and
  // invoice.payment_succeeded can all fire for the same purchase (see the webhook
  // handler's "second chance" comment) — without this, "already active" would
  // re-send the activation email on every one of those deliveries.
  const { data: existingSub } = await supabaseAdmin
    .from("subscriptions")
    .select("status")
    .eq("user_id", userId)
    .maybeSingle();
  const outcome = classifyActivation({ isNewUser, oldStatus: existingSub?.status });
  ...
  // the normal way afterward. A reactivating customer already has a way in and gets
  // a plain confirmation instead. Failure here must never undo the activation above:
  ...
  try {
    ...
    const result = await sendEmail({
      to: email,
      subject: outcome === "reactivated" ? "You're back on the Pro plan — VibeSafe Builder" : "You're on the Pro plan — VibeSafe Builder",
      html: proActivationEmailHtml({ name: null, setPasswordUrl, reactivated: outcome === "reactivated" }),
    });
...
// RFC 9116 security contact. express.static (and sendFile) skip dot-directories by
// default, so .well-known needs an explicit route to be served.
...
//
// The s-maxage matters more than it looks. Every other marketing page is a static
// file that Vercel's CDN serves in ~50ms as a HIT. This one is a route, so without
// a shared-cache directive it was x-vercel-cache: MISS on every single request -
// 300-500ms warm, and a cold function start is far worse. That is the slowest URL
// on the site, and it is the one a crawler hits first.
// max-age=0 keeps browsers revalidating, so a deploy is still visible immediately;
// Vercel starts each deployment with a cold CDN cache regardless.
app.get("/", (req, res) => {
...
// Abuse reporting. Reached from the badge injected into every published app,
// so this URL is effectively public API: it must not move.
...
app.post("/api/publish", async (req, res) => {
  ...
  // A published app must never contain {{image:...}}. If one gets this far the
  // browser failed to embed the image, and shipping literal braces onto a
  // customer's live page is worse than refusing the publish.
  if (kind !== "multi" && require("../public/assets.js").hasPlaceholder(code)) {
    return res.status(400).json({
      error: "This app still references an image that hasn't been embedded. Reopen the project, check the Images panel, and publish again.",
      stage: "validating",
    });
  }
  // Same reasoning for payment links: a live "Buy" button pointing at literal
  // braces takes a real buyer's click and goes nowhere.
  if (kind !== "multi" && require("../public/payments.js").hasPlaceholder(code)) {
    return res.status(400).json({
      error: "This app has a buy button with no payment link behind it. Reopen the project, check the Payments panel, and publish again.",
      stage: "validating",
    });
  }
...
app.get("/p/:id", async (req, res) => {
  ...
  const { data, error } = await supabase
    .from("published_apps")
    .select("html, disabled_at, disabled_reason")
    .eq("id", req.params.id)
    .single();
  if (error || !data) return res.status(404).send("App not found");
  // 451, not 404: the app existed and was removed. Saying so plainly is better
  // than pretending it never existed, and it keeps the URL out of search.
  if (data.disabled_at) {
    return res.status(451).set("Content-Type", "text/html; charset=utf-8").set("X-Robots-Tag", "noindex").send(takenDownPage(data.disabled_reason));
  }
  ...
  res.set(
    ...
    "default-src 'self' data: blob:; " +
      ...
      "font-src 'self' data: https://fonts.gstatic.com https://cdn.jsdelivr.net; " +
      "img-src 'self' data: blob: https://cdn.jsdelivr.net https://unpkg.com https://cdnjs.cloudflare.com; " +
      `connect-src 'self' ${new URL(SITE_URL).origin}/api/forms/public/ https://cdn.jsdelivr.net https://unpkg.com https://cdnjs.cloudflare.com https://esm.sh; ` +
      `object-src 'none'; base-uri 'self'; form-action 'self' ${new URL(SITE_URL).origin}/api/forms/public/; frame-ancestors 'self';`
  );
...
2. Optionally, when this app has a spec (it will be shown to you): a line starting SPEC+ recording
   one thing that must now stay true, and/or a line starting SPEC! describing how the request
   conflicts with an existing rule. At most one of each, never invented to look thorough.
3. A single fenced code block: \`\`\`html ... \`\`\` containing the COMPLETE updated HTML document.
...
CUSTOMER IMAGES:
The user can attach their own images to the project in three named slots: hero, logo and
background. Reference one by writing the placeholder as the image source — src="{{image:hero}}",
or url({{image:background}}) in CSS. The builder swaps the placeholder for the real image when
the app is previewed, published or downloaded, so never invent a file path, a URL, or base64
data of your own. Use a slot only when the user has asked for their image, or has clearly
supplied one for that purpose; otherwise design without photographs. Always give an
{{image:hero}} or {{image:logo}} img a real alt description, and keep the layout sensible when
an image is missing — a customer may remove it later.

SELLING SOMETHING:
The user can attach Stripe payment links to the project, each with a name and a price. Reference one
by writing its placeholder as the link target — <a href="{{pay:starter}}">. The builder swaps it for
the real Stripe checkout URL at preview, publish and download. Never invent a Stripe URL, never write
a real one you were shown elsewhere, and never build a card form, card fields, or any page that
collects card numbers: checkout happens on Stripe's own hosted page, which is the entire point. Use a
normal link or a link styled as a button, not a <form> that posts to Stripe — published apps restrict
form submissions to their own origin. Show the price next to the button in the app's own text, and do
not promise delivery, refunds or terms the user has not stated.
...
FILE UPLOADS (optional — when the app needs a photo, a CV, or a document from its visitors):
Send the file's bytes as the request body. Do NOT base64 it, and do NOT build multipart.
- Upload: POST {VIBESAFE_BACKEND_URL}/files
    headers: x-app-id, x-app-key, x-file-name: <the original name>, content-type: <the file's type>
    body: the raw File/Blob straight from the <input type="file">
    -> {id, name, mime, bytes, url}
- Show/download it: use the returned \`url\` directly, e.g. <img src="{url}"> or <a href="{url}" download>.
- Delete: DELETE {VIBESAFE_BACKEND_URL}/files/:id  (needs x-app-id and x-app-key)
Store the returned url alongside your own record, so the app knows which file belongs to what.
Accepted: PNG, JPEG, GIF, WebP and PDF only, 2MB each, 200 files per app. The type is decided by
inspecting the bytes, so renaming a file does not get it accepted — tell the visitor plainly what
is allowed, show upload progress, and handle 413 (too big) and 415 (wrong type) with a clear
message rather than a silent failure. Anyone with the link can view a file, so do not use this for
anything private.
...
app.get("/api/managed/status", async (req, res) => {
  ...
  const starter = !active && supabaseAdmin && process.env.ANTHROPIC_API_KEY
    ? await checkStarter(supabaseAdmin, user.id, starterConfig())
    : null;
  res.json({
    signedIn: true,
    email: user.email,
    ...
    starter: starter
      ? {
          available: starter.available,
          reason: starter.reason,
          buildsLeft: starter.buildsLeft,
          buildsTotal: starter.buildsTotal,
          model: STARTER_MODEL_LABEL,
          paidModel: PAID_MODEL_LABEL,
        }
      : null,
  });
});
...
// Routes password reset through Resend instead of Supabase's own auth email, using
// the same admin.generateLink() + sendEmail() pattern activateManagedAccess() already
// uses for a new customer's set-password link. Two reasons: consistent branding, and
// this Resend integration is verified working this session — Supabase's own SMTP
// configuration for auth emails has never been checked at all.
//
// The response is identical whether or not the address has an account, and no
// email is sent for one that doesn't — a reset endpoint that behaves differently
// for a real vs. unknown address lets anyone check who's signed up.
//
// No rate limiting on this endpoint yet — worth adding if it sees abuse, but this
// codebase has no rate-limiting middleware anywhere yet to reuse, and Supabase's own
// resetPasswordForEmail() (what this replaces) had no rate limit either, so this
// doesn't make the actual exposure worse than what was already there.
// Signup confirmation, sent by us rather than by Supabase - the last auth email
// that still depended on Supabase's own SMTP. lib/signup.js has the reasoning,
// the throttle that replaces the one Supabase's public endpoint gave us for
// free, and why every outcome returns the same response body.
...
app.post("/api/request-password-reset", async (req, res) => {
  ...
  if (!supabaseAdmin) return res.json(GENERIC); // degrade silently, same response either way
  ...
  try {
    ...
    if (userId) {
      ...
      if (linkError || !resetUrl) {
        console.error("Could not generate password-reset link for", email, linkError);
      } else {
        const result = await sendEmail({ to: email, subject: "Reset your password — VibeSafe Builder", html: passwordResetEmailHtml({ resetUrl }) });
        if (!result.ok) console.error("Could not send password-reset email to", email, result.error);
      }
    }
    // No account found: fall through to the same response, silently.
  } catch (err) {
    console.error("Password-reset request error for", email, err);
    // Still the generic response - a transient error here must not read as
...
// The rest of the owner dashboard: anonymous daily activity (so BYOK users, who
// never create an account, stop being invisible), Stripe revenue, and published
// apps. Each section loads and fails independently - see lib/adminStats.js.
app.get("/api/admin/stats", async (req, res) => {
  if (!requireAdmin(req, res)) return;
  if (!supabaseAdmin) {
    return res.status(503).json({ error: "Missing SUPABASE_SERVICE_ROLE_KEY — admin data isn't available on this server." });
  }
  const trial = starterConfig();
  res.json(
    await loadAdminStats({
      supabaseAdmin,
      stripe,
      stripeConfigured: !!STRIPE_SECRET_KEY,
      starterMonthlyCap: trial.enabled ? trial.monthlyCap : 0,
    })
  );
});
...
registerReportRoutes(app, { supabaseAdmin, sendEmail, siteUrl: SITE_URL, requireAdmin });
require('./forms').registerFormsRoutes(app, { supabaseAdmin, getManagedUser, sendEmail, siteName: 'VibeSafe Builder' });
...
async function resolveMode(req) {
  ...
  const key = req.header("x-anthropic-key");
  if (!key) {
    // Free trial (lib/starter.js): a signed-in account with no subscription and no key
    // of its own gets a small, capped number of builds on the platform key, on a
    // cheaper model. Only this branch can reach it - paying customers are handled
    // above, and anyone who supplies a key uses their own key.
    const serverKey = process.env.ANTHROPIC_API_KEY;
    if (managedUser && supabaseAdmin && serverKey) {
      const starter = await checkStarter(supabaseAdmin, managedUser.id, starterConfig());
      if (starter.available) {
        return { client: new Anthropic({ apiKey: serverKey }), mode: "starter", userId: managedUser.id, starter };
      }
      const message = starterUnavailableMessage(starter.reason);
      if (message) return { error: message, status: 401, reason: `starter_${starter.reason}` };
    }
...
function buildMessages(prompt, currentCode, strategy, palette, spec) {
  let text = "";
  if (currentCode) {
  ...
  // The spec goes in on EVERY build, not just the first. That is the whole
  // point: the model is reminded of what must stay true instead of being
  // trusted to remember it twenty edits later.
  text += appSpec.promptBlock(spec);
...
const { selectAgent, agentSystem, reviewResult } = require('./agents');
function agentMessages(agent, body) {
  const { prompt = '', currentCode = '', files = null, kind = 'single', strategy = null, palette = null, spec = null } = body;
  if (agent === 'review') return [{ role: 'user', content: JSON.stringify({ request: prompt, project: kind === 'multi' ? files : { 'index.html': currentCode } }) }];
  return kind === 'multi' ? buildMultiFileMessages(prompt, files || {}, strategy, palette) : buildMessages(prompt, currentCode, strategy, palette, spec);
}
...
app.post("/api/estimate", async (req, res) => {
  let agent;
  try { agent = selectAgent(req.body); } catch (err) { return res.status(400).json({ error: err.message }); }
  ...
  const { client, mode, usage, starter } = resolved;
  try {
    const { prompt = "", currentCode = "", kind = "single", files = null, palette = null } = req.body;
    const isMulti = kind === "multi";
    const pricing = mode === "starter" ? STARTER_PRICING : { input: INPUT_PER_MTOK, output: OUTPUT_PER_MTOK };
    const count = await client.messages.countTokens({
      model: mode === "starter" ? STARTER_MODEL : MODEL,
      system: agentSystem(agent, isMulti ? MULTIFILE_PROMPT : SYSTEM_PROMPT),
      messages: agentMessages(agent, req.body),
    });
    const inputTokens = count.input_tokens;
    // Output is roughly the size of the app being (re)generated, plus headroom for thinking.
    const codeTokens = agent === 'review' ? 1500 : Math.max(2500, Math.ceil((isMulti ? Object.values(files || {}).join('\n') : currentCode).length / 3.2));
    const estOutLow = Math.round(codeTokens * 0.9);
    const estOutHigh = Math.round(codeTokens * 2.2 + 2000);
    const low = (inputTokens / 1e6) * pricing.input + (estOutLow / 1e6) * pricing.output;
    const high = (inputTokens / 1e6) * pricing.input + (estOutHigh / 1e6) * pricing.output;
    const payload = { inputTokens, low, high, mode, agent };
    if (mode === "starter") {
      payload.starter = { model: STARTER_MODEL_LABEL, paidModel: PAID_MODEL_LABEL, buildsLeft: starter.buildsLeft };
    }
    if (mode === "managed") {
...
app.post("/api/generate", async (req, res) => {
  ...
  try {
    const stream = client.messages.stream({
      model: mode === "starter" ? STARTER_MODEL : MODEL,
      max_tokens: agent === 'review' ? 8000 : mode === "starter" ? STARTER_MAX_TOKENS : 64000,
      thinking: { type: "adaptive" },
      system: [
        {
          type: "text",
          text: agentSystem(agent, isMulti ? MULTIFILE_PROMPT : SYSTEM_PROMPT),
    ...
    const fullText = final.content
      ...
      .map((b) => b.text)
      .join("");
    const genCost = mode === "starter" ? costFor(final.usage, STARTER_PRICING) : costUSD(final.usage);
    ...
    const succeeded =
      ...
      (agent === 'review' ? Boolean(fullText.trim()) : isMulti ? Object.keys(changedFiles || {}).length > 0 : /<html[\s>]/i.test(fullText));
    ...
    if (mode === "starter") {
      try {
        await recordStarterUsage(supabaseAdmin, userId, genCost, succeeded);
      } catch (err) {
        // Never lose the user's build over bookkeeping; the monthly cap still bounds spend.
        console.error("Could not record free trial usage for", userId, err.message);
      }
    }
    send({
      type: "done",
      agent,
      ...reviewResult(agent, fullText),
      text: fullText,
      files: changedFiles,
      usage: final.usage,
      cost: genCost,
    ...
    // After "done" so the user never waits on it, but before res.end(): work after
    // the response closes is not guaranteed to run on serverless.
    await recordGenerationStat(supabaseAdmin, { mode, cost: genCost, succeeded });
  } catch (err) {
    send({ type: "error", message: describeError(err) });
    await recordGenerationStat(supabaseAdmin, { mode, cost: 0, succeeded: false });
  } finally {
    res.end();
  }
});
