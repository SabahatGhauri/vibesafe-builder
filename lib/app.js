...
const stripe = new Stripe(process.env.STRIPE_SECRET_KEY || "sk_placeholder_webhook_verify_only");
...
app.get("/api/admin/users", async (req, res) => {
  ...
  try {
    ...
    const [{ data: subs, error: subErr }, { data: usage, error: usageErr }] = await Promise.all([
      supabaseAdmin.from("subscriptions").select("user_id, status, stripe_customer_id, updated_at").in("user_id", ids.length ? ids : ["-"]),
      supabaseAdmin.from("managed_usage").select("user_id, dollars_spent, build_count").eq("period", currentPeriod()).in("user_id", ids.length ? ids : ["-"]),
    ]);
