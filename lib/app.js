...
app.post("/api/publish", async (req, res) => {
  ...
  // This runs AFTER the upsert above, not before: app_backend_keys.app_id has a foreign
  // key to published_apps(id), so the row must exist first or provisioning is rejected
  // on a brand-new app's first publish. Hence the second write to store the final HTML.
  if (supabaseAdmin) {
    try {
      ...
      // Never fail a publish over this — the app is already saved and served fine
      // above, just without the shared-data backend, which its own guard handles.
      console.error("Could not provision backend key for", publishId, err);
    }
  }
...
\`data\` is any JSON object, max 8KB, and each app is capped at 1000 records. There is NO
per-user login on this backend yet — every record in a collection is visible to every
visitor of the app. Do not build a "private data" or "password login" feature on top of it;
if asked for that specifically, say in your one-sentence reply that per-user accounts
