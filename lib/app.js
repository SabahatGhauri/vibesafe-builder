...
const stripe = new Stripe(STRIPE_SECRET_KEY || "sk_placeholder_webhook_verify_only");
...
async function activateManagedAccess(email, stripeCustomerId) {
  ...
  // this is what actually makes that true. admin.generateLink() deliberately does NOT
  // send anything (it just returns a link for a custom email flow); signInWithOtp() on
  // the anon client is what triggers Supabase's real magic-link email. Failure here
  // must never undo the activation above — the customer is still a paying, active
  // managed user even if this particular email attempt fails (e.g. rate-limited); they
  // can always request a fresh link themselves from the sign-in box.
  if (supabase) {
    try {
      const { error: otpError } = await supabase.auth.signInWithOtp({
        email,
        options: { shouldCreateUser: false, emailRedirectTo: `${SITE_URL}/app` },
      });
      if (otpError) console.error("Could not send managed-plan welcome email to", email, otpError);
    } catch (err) {
      console.error("Could not send managed-plan welcome email to", email, err);
    }
  }
}
...
// TEMPORARY KILL SWITCH (2026-08-16): a real customer's Stripe Checkout session got
// another product's existing customer record attached to it (Stripe Link cross-merchant
// autofill, on top of the two products still sharing one Stripe account) — disabling
// Managed checkout until that's understood and safe. Remove this block to re-enable;
// nothing else about /api/checkout was touched.
const MANAGED_CHECKOUT_DISABLED = true;
...
app.post("/api/publish", async (req, res) => {
  ...
  if (blockers.length) {
    return res.status(422).json({ error: "Blocked by security scan", blockers });
  }
  ...
  const html = injectPWATags(code, { swUrl: `/p/${publishId}/sw.js` });
  const { error } = await supabase
    .from("published_apps")
    .upsert({ id: publishId, html, updated_at: new Date().toISOString() });
  if (error) return res.status(500).json({ error: "Could not save the published app: " + error.message });
  res.json({ id: publishId, url: `/p/${publishId}` });
});
...
async function resolveMode(req) {
  ...
  if (managedUser) {
    if (!(await isSubscriptionActive(managedUser.id))) {
      return { error: "Your managed plan isn't active. Check your payment, or use your own API key instead.", status: 402 };
    }
  ...
  // reserved for signed-in Managed-plan users (budget-tracked above). Falling back to it
  // for anyone who omits x-anthropic-key would let any anonymous visitor generate on the
  // platform's own key for free, with no auth and no budget cap.
