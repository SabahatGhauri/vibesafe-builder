...
app.post("/api/publish", async (req, res) => {
  // Both clients are required now: supabaseAdmin does the writes (see the security
  // note at the PUBLISHING stage below), supabase serves the public read at /p/:id.
  ...
  // ---- stage: VALIDATING ----
  // Multi-file projects arrive as a file map and are assembled here rather than in
  // the browser, so the artifact that gets scanned and stored is produced by one
  // code path (lib/multifile.js) regardless of who asked for the publish.
  let publishCode = code;
  let scanTarget = code;
  if (isMulti) {
    const errs = validateFiles(files || {});
    if (errs.length) {
      return res.status(400).json({ error: "Invalid project: " + errs.join(" "), stage: "validating" });
    }
    try {
      publishCode = assembleProject(files);
    } catch (err) {
      return res.status(400).json({ error: "Could not assemble the project: " + err.message, stage: "building" });
    }
    // Crucially the scan reads the MODEL'S SOURCES, not the assembled bundle — the
    // bundle embeds our own new Function module loader, which the eval/new Function
    // blocker would otherwise flag on every single multi-file publish.
    scanTarget = concatSources(files);
  } else if (!/<html[\s>]/i.test(code) || code.length > 2_000_000) {
  ...
  const blockers = PUBLISH_BLOCKERS.filter((b) => b.re.test(scanTarget)).map((b) => b.label);
  if (blockers.length) {
  ...
  let html = injectPWATags(publishCode, { swUrl: `/p/${publishId}/sw.js` });

  // ---- stage: PUBLISHING (parent row first — see FK note below) ----
  // Writes go through the SERVICE-ROLE client, never the anon one. Publishing used
  // to write with `supabase` (anon), which forced published_apps to carry
  // `insert with check (true)` / `update using (true)` policies — and since the anon
  // key is public (served to every browser by /api/config), that let anyone
  // overwrite anyone else's published app, then act as that app against
  // /api/backend using its injected key. See docs/security-notes.md.
...
app.post("/api/preview-link", async (req, res) => {
  ...
  const { files = null, id = null } = req.body || {};
  const errs = validateFiles(files || {});
  if (errs.length) return res.status(400).json({ error: errs.join(" ") });

  const blockers = PUBLISH_BLOCKERS.filter((b) => b.re.test(concatSources(files))).map((b) => b.label);
  if (blockers.length) return res.status(422).json({ error: "Blocked by security scan", blockers });

  const previewId = id && /^pv[a-zA-Z0-9_-]{4,22}$/.test(id) ? id : "pv" + randomId();
  let html;
  try {
  ...
  const { error } = await supabaseAdmin
    .from("published_apps")
    .upsert({ id: previewId, html, updated_at: new Date().toISOString() });
  if (error) return res.status(500).json({ error: "Could not save the preview: " + error.message });
  res.json({ id: previewId, url: "/p/" + previewId });
});
...
function buildMultiFileMessages(prompt, files, strategy) {
  let text = "";
...
app.post("/api/estimate", async (req, res) => {
  ...
  try {
    const { prompt = "", currentCode = "", kind = "single", files = null } = req.body;
    const isMulti = kind === "multi";
    const count = await client.messages.countTokens({
      model: MODEL,
      system: isMulti ? MULTIFILE_PROMPT : SYSTEM_PROMPT,
      messages: isMulti ? buildMultiFileMessages(prompt, files || {}, null) : buildMessages(prompt, currentCode, null),
    });
...
app.post("/api/generate", async (req, res) => {
  ...
  const { prompt = "", currentCode = "", strategy = null, kind = "single", files = null } = req.body;
  const isMulti = kind === "multi";
  ...
  try {
    const stream = client.messages.stream({
      ...
      messages: isMulti
        ? buildMultiFileMessages(prompt, files || {}, strategy)
        : buildMessages(prompt, currentCode, strategy),
    });
