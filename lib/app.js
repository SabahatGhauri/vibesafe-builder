...
function agentMessages(agent, body) {
  const { prompt = '', currentCode = '', files = null, kind = 'single', strategy = null, palette = null } = body;
  if (agent === 'review') return [{ role: 'user', content: JSON.stringify({ request: prompt, project: kind === 'multi' ? files : { 'index.html': currentCode } }) }];
  return kind === 'multi' ? buildMultiFileMessages(prompt, files || {}, strategy, palette) : buildMessages(prompt, currentCode, strategy, palette);
}
