...
const vibesafe = {
  ...
  async status() {
    ...
    return { connected: true, account: "VibeSafe Builder Hosting" };
  },
  ...
  async deploy(ctx, { artifact, environment = "production", appId }) {
    ...
    // Published apps are served from our domain, so every one carries a way to
    // report it. Injected here rather than generated, for the same reason the
    // PWA tags are: it must not depend on the model remembering. Downloads skip
    // this - a file someone owns should not carry our badge.
    if (ctx.injectReportBadge) html = ctx.injectReportBadge(html, { appId: id, siteUrl: ctx.siteUrl });
