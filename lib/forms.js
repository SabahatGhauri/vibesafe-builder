...
// The confirmation a submitter receives. Written by the form's owner, stored
// with the form, never taken from the request - the submitter controls only
// the address it goes to, not a single character of what it says.
const REPLY_SUBJECT_MAX = 120;
const REPLY_BODY_MAX = 500;

function validateReplySettings(body) {
  const enabled = body.replyEnabled === true;
  if (!enabled) return { reply_enabled: false, reply_subject: null, reply_body: null };
  const subject = String(body.replySubject || '').replace(/[\r\n]+/g, ' ').trim().slice(0, REPLY_SUBJECT_MAX);
  const text = String(body.replyBody || '').replace(/\r/g, '').trim().slice(0, REPLY_BODY_MAX);
  if (!subject) throw new Error('Give the confirmation email a subject line.');
  if (!text) throw new Error('Write the message people will receive.');
  return { reply_enabled: true, reply_subject: subject, reply_body: text };
}

// Plain text escaped into HTML: the owner writes prose, not markup, and their
// own account being compromised should not turn into markup in someone's inbox.
function replyHtml(form, siteName) {
  const body = escapeHtml(form.reply_body).replace(/\n/g, '<br>');
  return (
    `<div style="font:15px/1.6 system-ui,-apple-system,sans-serif;color:#111">` +
    `<p>${body}</p>` +
    `<hr style="border:0;border-top:1px solid #ddd;margin:18px 0">` +
    `<p style="color:#666;font-size:12.5px">You received this because you submitted the ` +
    `“${escapeHtml(form.name)}” form. If that wasn't you, ignore this message — ` +
    `nobody can subscribe you to anything here, and you will not be emailed again.</p>` +
    `<p style="color:#999;font-size:11.5px">Sent via ${escapeHtml(siteName || 'VibeSafe Builder')} on behalf of the form's owner.</p>` +
    `</div>`
  );
}
...
function registerFormsRoutes(app, { supabaseAdmin: db, getManagedUser, sendEmail, siteName }) {
  ...
  publicRouter.post('/:id/submit', wrap(async(req,res) => {
    ...
    if(saved.notify) {
      let delivered = false;
      try {
        const result = await sendEmail({to:saved.email,subject:'New VibeSafe Builder Forms submission',idempotencyKey:`native-form-${saved.id}`,timeoutMs:10000,html:submissionHtml(saved.name,fields)});
    ...
    // Confirmation to the submitter. Deliberately last: the submission is
    // already stored and the owner already notified, so a failure here costs
    // nobody anything. Rate-limited per address in the database so the same
    // address cannot be used to amplify a message at someone.
    if(saved.replyEnabled) {
      try {
        const {data:allowed} = await db.rpc('may_send_form_reply',{p_form:req.params.id,p_email:fields.email,p_submission:saved.id});
        if(allowed === true) {
          let delivered = false;
          try {
            const result = await sendEmail({
              ...
              idempotencyKey: `native-form-reply-${saved.id}`,
              timeoutMs: 10000,
              html: replyHtml({name:saved.name, reply_body:saved.replyBody}, siteName),
            });
            delivered = result.ok === true;
          } catch { /* reported below */ }
          try { await db.from('native_form_submissions').update({reply_status:delivered?'sent':'failed'}).eq('id',saved.id); } catch {}
        }
      } catch { /* a confirmation is a courtesy; never fail the submission over it */ }
    }
  ...
  owner.post('/',wrap(async(req,res) => {
    ...
    let reply;
    try { reply = validateReplySettings(b); }
    catch(e) { return res.status(400).json({error:e.message}); }
    ...
    if(reply.reply_enabled) {
      try { await db.from('native_forms').update(reply).eq('id',data.id).eq('owner_id',req.formOwner.id); }
      catch { /* the form exists; the confirmation can be switched on later */ }
    }
    res.status(201).json({form:{id:data.id,name:data.name,kind:data.kind,notify:data.notify,enabled:data.enabled,replyEnabled:reply.reply_enabled}});
  }));
  ...
  owner.post('/:id/submissions/:sid/notify',wrap(async(req,res) => {
    ...
    try {
      ...
      const result = await sendEmail({to:form.notification_email,subject:'New VibeSafe Builder Forms submission',idempotencyKey:`native-form-resend-${row.id}`,timeoutMs:10000,html:submissionHtml(form.name,row.data)});
      delivered = result.ok === true;
...
module.exports = { registerFormsRoutes, validateSubmission, csvCell, validateReplySettings, replyHtml };
