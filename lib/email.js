...
async function sendEmail({ to, subject, html, idempotencyKey, timeoutMs }) {
  if (!RESEND_API_KEY) {
...
// The confirmation that used to come from Supabase's own sender. Same job,
// our domain, and it no longer depends on the project's SMTP configuration
// being right - see lib/signup.js for why that mattered.
function signupConfirmEmailHtml({ confirmUrl, name }) {
  const greeting = name ? `Hey ${escapeHtml(name)},` : "Hey there,";
  return shell(`
    <h1 style="color:#eef3f7;font-size:20px;margin:0 0 12px;">Confirm your account</h1>
    <p style="color:#eef3f7;font-size:14px;line-height:1.6;margin:0 0 16px;">${greeting}</p>
    <p style="color:#8d9aa8;font-size:14px;line-height:1.6;margin:0 0 20px;">
      One click and your VibeSafe Builder account is ready. Your first app is free — no card, no API key.
    </p>
    ${button(confirmUrl, "Confirm my account")}
    <p style="color:#5f6b78;font-size:12.5px;line-height:1.6;margin:20px 0 0;">
      If you didn't sign up, you can ignore this email and nothing further will happen.
    </p>
  `);
}

// Sent when someone tries to sign up with an address that already has an
// account. The signup response itself cannot say so without turning the form
// into a way to check who has registered, so the answer goes to the person who
// owns the inbox instead - with a way back in, since forgetting is the usual
// reason to end up here.
function existingAccountEmailHtml({ resetUrl }) {
  return shell(`
    <h1 style="color:#eef3f7;font-size:20px;margin:0 0 12px;">You already have an account</h1>
    <p style="color:#8d9aa8;font-size:14px;line-height:1.6;margin:0 0 16px;">
      Someone just tried to sign up to VibeSafe Builder with this address, and it already has an account.
      If that was you, log in as normal — or set a new password here if you've forgotten it:
    </p>
    ${button(resetUrl, "Set a new password")}
    <p style="color:#5f6b78;font-size:12.5px;line-height:1.6;margin:20px 0 0;">
      If it wasn't you, nothing has changed and your password is untouched. No new account was created.
    </p>
  `);
}
