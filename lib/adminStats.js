...
// "starter" arrived with the free trial, after this module was written. It was
// missing here, so every trial build recorded nothing at all.
const STAT_MODES = ["byok", "managed", "starter"];
...
function summarizeDailyStats(rows, { days = 30, now = new Date() } = {}) {
  ...
  for (let i = 0; i < days; i++) {
    const point = { day: utcDay(new Date(start.getTime() + i * DAY_MS)), byok: 0, managed: 0, starter: 0, failures: 0 };
  ...
  const activeDays = series.filter((p) => p.byok + p.managed + p.starter + p.failures > 0).length;
...
// The production smoke test (test/smoke.js) publishes real apps whose ids always
// start with "smoke". They are not customers, so every figure below excludes them
// and they are counted separately, rather than silently dropped.
const TEST_APP_PATTERN = "smoke%";
...
async function fetchPublishedApps(supabaseAdmin, { now = new Date(), recentLimit = 10 } = {}) {
  const count = (sinceDays, { testApps = false } = {}) => {
    let q = supabaseAdmin.from("published_apps").select("id", { count: "exact", head: true });
    q = testApps ? q.like("id", TEST_APP_PATTERN) : q.not("id", "like", TEST_APP_PATTERN);
    if (sinceDays) q = q.gte("created_at", new Date(now.getTime() - sinceDays * DAY_MS).toISOString());
    return q;
  };
  const [total, last7, last30, tests, recent] = await Promise.all([
    count(null),
    count(7),
    count(30),
    count(null, { testApps: true }),
    supabaseAdmin
      .from("published_apps")
      .select("id, created_at, updated_at")
      .not("id", "like", TEST_APP_PATTERN)
      .order("updated_at", { ascending: false })
      .limit(recentLimit),
  ]);
  for (const r of [total, last7, last30, tests, recent]) if (r.error) throw r.error;
  return {
    available: true,
    total: total.count || 0,
    last7: last7.count || 0,
    last30: last30.count || 0,
    testAppsHidden: tests.count || 0,
    recent: (recent.data || []).map((a) => ({ id: a.id, createdAt: a.created_at, updatedAt: a.updated_at })),
  };
}

// Free trial spend this month, from the per-account "starter-YYYY-MM" rows written
// by lib/starter.js. Aggregates only; no account is identified here.
async function fetchStarter(supabaseAdmin, { now = new Date(), monthlyCap = null } = {}) {
  const period = `starter-${now.getUTCFullYear()}-${String(now.getUTCMonth() + 1).padStart(2, "0")}`;
  const { data, error } = await supabaseAdmin
    .from("managed_usage")
  ...
  return {
    ...
    builds: rows.reduce((n, r) => n + (r.build_count || 0), 0),
    spent: rows.reduce((n, r) => n + (Number(r.dollars_spent) || 0), 0),
    monthlyCap,
  };
}
...
async function loadAdminStats({ supabaseAdmin, stripe, stripeConfigured, starterMonthlyCap = null, now = new Date() }) {
  const [activity, revenue, apps, starter] = await Promise.all([
    ...
    settle(() => fetchStarter(supabaseAdmin, { now, monthlyCap: starterMonthlyCap })),
  ]);
  return { generatedAt: now.toISOString(), activity, revenue, apps, starter };
}
