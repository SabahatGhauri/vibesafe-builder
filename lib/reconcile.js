...
async function reconcile({ stripe, supabaseAdmin, activate, apply = false }) {
  const [stripeSubs, dbRows, account] = await Promise.all([
    fetchStripeSubscriptions(stripe),
    loadDbRows(supabaseAdmin),
    // Which Stripe account this key actually belongs to. Two accounts exist for
    // this business and have been confused before; a report saying "no gap" is
    // meaningless if it silently queried the wrong one.
    // Defensive: this is diagnostic only, so a Stripe client without accounts
    // access, or a failing lookup, must never take reconciliation down with it.
    (stripe.accounts && stripe.accounts.retrieve
      ? stripe.accounts.retrieve().then(
          (a) => ({ id: a.id, name: a.business_profile?.name || a.settings?.dashboard?.display_name || null }),
          () => null
        )
  ...
  return {
    ...
    // Every status seen, so a subscription sitting in past_due or incomplete is
    // visible rather than just silently absent from the entitled count.
    stripeStatusCounts: stripeSubs.reduce((acc, s) => { acc[s.status] = (acc[s.status] || 0) + 1; return acc; }, {}),
    rowsInDb: dbRows.length,
    ok: diff.ok.map((s) => ({ customerId: s.customerId, email: s.email })),
