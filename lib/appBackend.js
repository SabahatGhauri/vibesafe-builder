...
function registerAppBackendRoutes(app, { supabaseAdmin }) {
  ...
  // Files register BEFORE the blanket auth below: uploads and deletes take the
  // app key, but reading a file must not - an <img src> cannot send a header,
  // and a file on a public page is as public as the page.
  require("./appFiles").registerAppFileRoutes(router, { supabaseAdmin, requireAppAuth, express });
