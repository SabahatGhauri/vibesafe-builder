...
const { scanProject, gateDecision, auditSummary } = require("./securityGate");
...
function registerVercelRoutes(app, { supabaseAdmin, getManagedUser }) {
  ...
  // Runs Launch Check without letting a scanner exception escape into Express.
  // This app is on Express 4 with no global error handler, and Node 24
  // terminates the whole process on an unhandled rejection from an async route
  // handler — a throw here would otherwise take the entire server down for
  // every user over ONE malformed project, not just fail the one request
  // (reproduced directly: an unguarded throw in this exact route shape kills
  // the process, exit code 1, before any response is sent).
  //
  // So a scan that cannot complete is treated exactly like a scan that found a
  // critical problem: BLOCKED, never passed. "Launch Check crashed" and "Launch
  // Check found nothing" must never look the same to the caller.
  function safeGate(files, { override }) {
    try {
      const scan = scanProject(files);
      const decision = gateDecision(scan, { override });
      return { ok: true, scan, decision };
    } catch (err) {
      console.error("Launch Check crashed while scanning a project:", err);
  ...
  app.post("/api/vercel/deploy", async (req, res) => {
    ...
    // Launch Check runs BEFORE anything reaches Vercel. A deployment that has
    // already happened cannot be un-happened: the files are on a CDN and, if
    // the project is public, potentially already crawled.
    const gated = safeGate(files, { override: Boolean(req.body.acknowledgeWarnings) });
    if (!gated.ok) {
      await record(user.id, "deploy_blocked", {
        project_id: c.connection.project_id,
        project_name: c.connection.project_name,
        environment,
        detail: { launchCheckError: true },
      });
      return res.status(422).json({
        blocked: true,
        reason: "Launch Check couldn't complete, so this wasn't deployed. Try again in a moment.",
        overridable: false,
      });
    }
    const { scan, decision } = gated;
    if (!decision.allowed) {
      await record(user.id, "deploy_blocked", {
        project_id: c.connection.project_id,
        project_name: c.connection.project_name,
        environment,
        detail: { launchCheck: auditSummary(scan) },
      });
      return res.status(422).json({
        blocked: true,
        reason: decision.reason,
        overridable: decision.overridable,
        scan: { verdict: scan.verdict, counts: scan.counts, findings: scan.findings },
      });
    }
    ...
    try {
      ...
      await record(user.id, "deploy", {
        ...
        url: dep.url,
        environment,
        detail: { fileCount: Object.keys(files).length, launchCheck: auditSummary(scan) },
      });
  ...
  app.post("/api/vercel/protection", async (req, res) => {
    ...
    // Launch Check gates going PUBLIC, not going private — putting protection
    // back is always safe and must never be blocked by a scan.
    //
    // The project is re-scanned here rather than trusting the result recorded
    // at deploy time. The files can have changed since, and "it passed once"
    // is exactly the reasoning that lets a secret through. This is also why the
    // route requires the files: without them there is nothing to check, and the
    // safe answer to "I cannot check" is no.
    let scan = null;
    if (action === "disable") {
      const files = req.body && req.body.files;
      if (!files || typeof files !== "object" || !Object.keys(files).length) {
        return res.status(400).json({
          error: "Can't run Launch Check without the project files, so this can't be made public.",
        });
      }
      const gated = safeGate(files, { override: Boolean(req.body.acknowledgeWarnings) });
      if (!gated.ok) {
        await record(user.id, "make_public_blocked", {
          project_id: c.connection.project_id,
          project_name: c.connection.project_name,
          detail: { launchCheckError: true },
        });
        return res.status(422).json({
          blocked: true,
          reason: "Launch Check couldn't complete, so this wasn't made public. Try again in a moment.",
          overridable: false,
        });
      }
      scan = gated.scan;
    ...
    await record(user.id, action === "disable" ? "protection_disabled" : "protection_enabled", {
      ...
      detail: {
        ...
        // What the gate saw at the moment this was allowed through.
        launchCheck: scan ? auditSummary(scan) : null,
      },
    });
