...
// Domain-separated so a GitHub ciphertext can never be decrypted by another
// integration deriving from the same server secret. Implementation lives in
// lib/secrets.js, shared with the Vercel credential.
const { makeCipher } = require("./secrets");
const cipher = makeCipher("vibesafe-github-tokens", "GITHUB_TOKEN_SECRET");
const encrypt = (v) => cipher.encrypt(v);
const decrypt = (v) => cipher.decrypt(v);

// The OAuth state is signed with the same derived key.
