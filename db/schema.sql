...
/* ======================== GitHub sync (Phase 2D) ======================== */

-- One connection per user. Tokens are stored ENCRYPTED (AES-256-GCM, see
-- lib/github.js), not merely RLS-protected: a `repo`-scoped GitHub token grants
-- write access to the user's repositories, so a database dump alone must not
-- yield anything usable. The encryption key never lives in the database.
...
-- Which repo a project is linked to, so push/pull need not be told every time.
